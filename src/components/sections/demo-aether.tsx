    ...
    <div className="relative w-screen h-screen bg-gray-900 font-sans overflow-hidden">
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-full max-w-md p-4">
        <div className="bg-black/40 backdrop-blur-lg p-6 rounded-2xl shadow-2xl space-y-4 border border-white/10">
          <ControlSlider label="Hue" value={hue} onChange={(e: any) => setHue(parseFloat(e.target.value))} min="0" max="360" step="1" />
          <ControlSlider label="Speed" value={speed} onChange={(e: any) => setSpeed(parseFloat(e.target.value))} min="0.0" max="1.0" step="0.01" />
          <ControlSlider label="Intensity" value={intensity} onChange={(e: any) => setIntensity(parseFloat(e.target.value))} min="0.1" max="2.0" step="0.01" />
          <ControlSlider label="Complexity" value={complexity} onChange={(e: any) => setComplexity(parseFloat(e.target.value))} min="1.0" max="8.0" step="0.1" />
          <ControlSlider label="Warp" value={warp} onChange={(e: any) => setWarp(parseFloat(e.target.value))} min="0.0" max="1.0" step="0.01" />
        </div>
      </div>
    </div>
  );
